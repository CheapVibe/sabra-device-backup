...
class ReportDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        failed_current = current_snapshots.filter(status='failed').count()
        ...
        failed_prev = previous_snapshots.filter(status='failed').count()
        ...
        devices_failed = Device.objects.filter(last_backup_status='failed', is_active=True).count()
