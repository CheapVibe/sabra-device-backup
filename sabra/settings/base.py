...
Sensitive settings (SECRET_KEY, FERNET_KEYS, DATABASE) are defined
in local.py which is generated by setup.sh during installation.

NOTE: This application requires setup.sh to be run before first use.
      Running without setup.sh will result in configuration errors.
"""
...
# DEBUG is overridden to False in production.py
DEBUG = True
...
# NOTE: FERNET_KEYS is defined in local.py (generated by setup.sh)
# Required for encrypting credentials in database
